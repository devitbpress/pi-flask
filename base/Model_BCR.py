...
def Model_Inventori_BCR(Harga_Komponen_Ho, Kerugian_Komponen_Co, Suku_bunga_i, Waktu_sisa_operasi=5, probabilitas="uniform"):
    ...
    Hasil_hitung_model_BCR = pd.DataFrame(df_hasil_list)
    hasil_hitung_final_model_BCR = pd.DataFrame(final_hasil_list)

    # Mengatur format tampilan angka dengan pemisah ribuan
    pd.options.display.float_format = '{:,.4f}'.format

    # Print tabel dataframe dengan format yang diatur
    print("\nTabel Hasil Perhitungan Model BCR:")
    display(Hasil_hitung_model_BCR)

    # Mengatur format tampilan angka dengan pemisah ribuan
    pd.options.display.float_format = '{:,.3f}'.format

    print("\nTabel Rekap Hasil Perhitungan Model BCR")
    display(hasil_hitung_final_model_BCR)

    # Mengembalikan hasil
    return Hasil_hitung_model_BCR, hasil_hitung_final_model_BCR
...
Hasil_hitung_model_BCR_kuadratis, hasil_hitung_final_model_BCR_kuadratis = Model_Inventori_BCR(Harga_Komponen_Ho, Kerugian_Komponen_Co, Suku_bunga_i, probabilitas="kubik", Waktu_sisa_operasi=20)
